...
from execution_factory import resolve_data_market
...
def _build_state_from_window(df_window: pd.DataFrame, latest_indicators: dict[str, Any], spread_pct: float) -> dict[str, Any]:
    last = df_window.iloc[-1]
    prev = df_window.iloc[-2] if len(df_window) > 1 else last
    day_open = float(df_window.iloc[0]["open"])
    control_zone = _safe_float(latest_indicators.get("vwap"), _safe_float(latest_indicators.get("bb_mid"), day_open))
    return {
...
def _prepare_history_bundle(market: MarketData, symbol: str, base_tf: str, timeframes: list[str], history_limit: int) -> dict[str, pd.DataFrame]:
...
def _build_mtf_context(bundle: dict[str, pd.DataFrame], cutoff_ts: pd.Timestamp, tfs: list[str]) -> dict[str, dict[str, Any]]:
...
def _close_position(position: dict[str, Any], exit_price: float, exit_time: pd.Timestamp, exit_type: str, bars_held: int, fee_rate: float) -> BacktestTrade:
    ...
    pnl_pct = ((exit_price - entry) / entry if side == "LONG" else (entry - exit_price) / entry) * 100.0
...
def _process_open_position(position: dict[str, Any], bar: pd.Series, fee_rate: float, config: dict[str, Any]) -> tuple[dict[str, Any] | None, BacktestTrade | None]:
    high = float(bar["high"])
    low = float(bar["low"])
    ...
    if side == "LONG":
        ...
        if tp_hit and sl_hit:
            trade = _close_position(position, sl, ts, "STOP_LOSS", position.get("bars_held", 0) + 1, fee_rate)
            return None, trade
        if sl_hit:
            trade = _close_position(position, sl, ts, "TRAIL_SL" if trail_armed else "STOP_LOSS", position.get("bars_held", 0) + 1, fee_rate)
            return None, trade
        if tp_hit:
            trade = _close_position(position, tp, ts, "TAKE_PROFIT", position.get("bars_held", 0) + 1, fee_rate)
            return None, trade
...
class BacktestEngine:
    ...
    def run(self) -> BacktestSummary:
        ...
        for i in range(warmup, len(base_df) - 1):
            ...
            if pending_entry is not None:
                ...
                entry_price = _entry_fill_price(float(next_bar["open"]), side, self.spread_pct, self.slippage_pct)
                amount = (pending_entry["trade_usdt"] * pending_entry["leverage"]) / entry_price
                position = {
                    ...
                    "trail_armed": False,
                    "sl": float(pending_entry["signal"].get("sl", entry_price * (0.996 if side == "BUY" else 1.004)) or entry_price),
                    "tp": float(pending_entry["signal"].get("tp", entry_price * (1.0025 if side == "BUY" else 0.9975)) or entry_price),
                    "break_even_trigger_pct": float(self.execution_cfg.get("break_even_trigger_pct", 0.0015) or 0.0015),
                    "break_even_buffer_pct": float(self.execution_cfg.get("break_even_buffer_pct", 0.0004) or 0.0004),
                    "trail_tighten_1_pct": float(self.execution_cfg.get("trail_tighten_1_pct", 0.0020) or 0.0020),
                    "trail_tighten_2_pct": float(self.execution_cfg.get("trail_tighten_2_pct", 0.0040) or 0.0040),
                    "trail_t1_gap_pct": float(self.execution_cfg.get("trail_t1_gap_pct", 0.0025) or 0.0025),
                    "trail_t2_gap_pct": float(self.execution_cfg.get("trail_t2_gap_pct", 0.0020) or 0.0020),
                    "min_hold_seconds": int(self.execution_cfg.get("scalp_min_hold_seconds", 10) or 10),
                    "bars_held": 0,
                    "signal_score": float(pending_entry["signal"].get("score", 0.0) or 0.0),
                    "signal_confidence": float(pending_entry["signal"].get("confidence", 0.0) or 0.0),
                    "signal_reason": str(pending_entry["signal"].get("reason", "") or ""),
                    "entry_mode": str(pending_entry.get("entry_mode", "") or ""),
                }
                pending_entry = None

            if position is not None:
                position, closed_trade = _process_open_position(position, bar, self.fee_rate, self.execution_cfg)
            ...
            mtf_context = _build_mtf_context(self.history_bundle, cutoff_ts, self.mtf_timeframes)
            pivots = _build_pivot_data(self.history_bundle, cutoff_ts)
            latest_macro = _latest_macro_from_context(
                mtf_context,
                funding_rate=float(self.market.fetch_funding_rate(self.symbol) or 0.0)
                if self.use_live_funding_rate and i == warmup
                else self.backtest_funding_rate,
            )
            state = _build_state_from_window(base_window, latest_indicators, self.spread_pct)
            ...
            if position is not None and self.allow_reversal:
                ...
                if (current_side == "LONG" and action == "SELL") or (current_side == "SHORT" and action == "BUY"):
                    ...
                    profit_pct = (current_price - entry) / entry if current_side == "LONG" else (entry - current_price) / entry
                    net_edge = float(self.execution_cfg.get("reversal_min_net_edge_pct", 0.0020) or 0.0020)
                    min_hold_seconds = float(position.get("min_hold_seconds", 0.0) or 0.0)
                    if profit_pct >= net_edge and float(position.get("hold_seconds", 0.0) or 0.0) >= min_hold_seconds:
                        closed_trade = _close_position(position, current_price, bar["timestamp"], "REVERSAL_BANK", position.get("bars_held", 0), self.fee_rate)
                        self.trades.append(closed_trade)
                        balance += closed_trade.pnl
                        position = None

            if signal.get("action") in {"BUY", "SELL"} and position is None and pending_entry is None:
                confidence = float(signal.get("confidence", 0.0) or 0.0)
                score = float(signal.get("score", 0.0) or 0.0)
                min_conf = float(self.strategy_cfg.get("min_conf", 0.15) or 0.15)
                if confidence >= min_conf and abs(score) >= float(self.strategy_cfg.get("entry_min_confidence_hard", 0.20) or 0.20):
                    trade_usdt = min(self.fixed_trade_usdt, balance * 0.90) if self.fixed_trade_usdt > 0 else balance * 0.25
                    if trade_usdt >= 10.0:
                        leverage = _dynamic_leverage_from_confidence(self.cfg, confidence)
                        pending_entry = {
                            "trade_id": trade_id,
                            "side": signal["action"],
                            ...
                            "leverage": leverage,
                            "entry_mode": _extract_entry_mode(str(signal.get("reason", "") or "")),
                        }
                        trade_id += 1
        ...
        if position is not None:
            ...
            closed_trade = _close_position(position, float(final_bar["close"]), final_bar["timestamp"], "EOD_CLOSE", position.get("bars_held", 0), self.fee_rate)
            self.trades.append(closed_trade)
            balance += closed_trade.pnl
    ...
    def _summarize(self, ending_balance: float, max_drawdown: float) -> BacktestSummary:
        ...
        summary.gross_profit = sum(t.pnl for t in self.trades if t.pnl > 0)
        summary.gross_loss = sum(t.pnl for t in self.trades if t.pnl < 0)
        summary.net_pnl = sum(t.pnl for t in self.trades)
        summary.profit_factor = (summary.gross_profit / abs(summary.gross_loss)) if summary.gross_loss < 0 else 0.0
        summary.expectancy = (summary.net_pnl / summary.trades) if summary.trades else 0.0
        ...
        summary.avg_hold_bars = (
            sum(t.bars_held for t in self.trades) / len(self.trades) if self.trades else 0.0
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a conservative candle-by-candle backtest using live signal logic.")
    parser.add_argument("--config", default="config.live.yaml", help="Config file to load")
    parser.add_argument("--symbol", default=None, help="Trading symbol override")
    parser.add_argument("--timeframe", default=None, help="Base timeframe override")
    parser.add_argument("--limit", type=int, default=1000, help="Historical candles per timeframe")
    parser.add_argument("--trades-out", default=None, help="Optional CSV path for trade log export")
    parser.add_argument("--equity-out", default=None, help="Optional CSV path for equity curve export")
    args = parser.parse_args()

    cfg = load_config(args.config)
    engine = BacktestEngine(cfg, symbol=args.symbol, timeframe=args.timeframe, limit=args.limit)
    engine.load_data()
    summary = engine.run()
    ...
    if args.trades_out:
        pd.DataFrame([asdict(t) for t in trades]).to_csv(args.trades_out, index=False)
